...
<th>'.$fonk->getPDil("Kargo Takip Link").'</th>
<th>#</th>
</tr>
</thead>
<tbody>';
foreach($siparisDurumlar as $siparisDurum){
	...
	<td><a href="'.$siparisDurum['siparisSiparisDurumKargoTakipLink'].'" target="_blank">'.$siparisDurum['siparisSiparisDurumKargoTakipLink'].'</a></td>
	<td>
					...
					<form id="formpost" class="form" action="" method="post">
						<table class="table table-bordered table-striped">
							...
							<tbody>
								...
								<tr id="takipLinkTr" style="display: none">
									<td style="width: 27%;vertical-align: middle;"><b><?=$fonk->getPDil("Kargo Takip Link")?></b></td>
									<td>
										<input type="text" class="form-control border-primary" id="siparisSiparisDurumKargoTakipLink" name="siparisSiparisDurumKargoTakipLink" value="<?=$Listeleme['siparisSiparisDurumKargoTakipLink']?>" autocomplete="off">
									</td>
								</tr>
								<tr>
									<td colspan="2" style="text-align:center">
										<?php if ($duzenlemeYetki==true) { ?>
											<input type="hidden" name="siparisSiparisDurumSiparisId" value="<?=$detay["siparisId"]?>" />
